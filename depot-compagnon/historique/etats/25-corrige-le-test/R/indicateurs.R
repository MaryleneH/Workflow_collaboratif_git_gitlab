...
#' Grande catégorie de PCS
#'
#' @param pcs codes PCS de travail (37, 47, 54, 62, 67).
#' @return la grande catégorie : les ouvriers qualifiés (62) et non qualifiés (67)
#'   sont regroupés.
regrouper_pcs <- function(pcs) {
  groupes <- c("37" = "cadres et ingénieurs", "47" = "techniciens et agents de maîtrise", "54" = "employés", "62" = "ouvriers", "67" = "ouvriers")
  unname(groupes[as.character(pcs)])
}

#' Répartition des salariés par grande catégorie de PCS dans chaque établissement
#'
#' @param salaries table des salariés, avec les colonnes `id_etab` et `pcs`.
#' @return une ligne par établissement et par grande catégorie, avec l'effectif
#'   et la part ; dans chaque établissement, les parts somment à 1.
