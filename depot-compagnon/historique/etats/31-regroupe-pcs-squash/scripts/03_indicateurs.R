...
ouvriers <- parts_pcs[parts_pcs$groupe_pcs == "ouvriers", ]
...
regions <- utils::read.csv(file.path("data", "reference", "regions.csv"),
                           colClasses = "character", encoding = "UTF-8")
par_region <- etablissements_par_region(salaries)
par_region$region <- regions$libelle_region[match(par_region$region, regions$region)]
indicateurs <- c(indicateurs, list(
  en_ligne(par_region, "part", "part_etablissements", "region")
))
